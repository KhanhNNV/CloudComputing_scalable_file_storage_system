...
import { FileText, Image as ImageIcon, File as FileIcon, Plus, Loader2, FolderPlus, Folder, ChevronRight, MoreVertical, Trash2, Download, Search } from 'lucide-react';
...
export default function Drive() {
  ...
  const handleDownloadFile = async (e, file) => {
    ...
    try {
      ...
      const downloadUrl = response?.url || response?.data?.url || (typeof response === 'string' ? response : null);
  ...
  const getFileIcon = (file) => {
    ...
    if (mimeType.includes('pdf') || name.endsWith('.pdf')) return <FileText className="w-8 h-8 text-red-500" />;
    if (mimeType.includes('word') || mimeType.includes('document') || name.endsWith('.doc') || name.endsWith('.docx')) return <FileText className="w-8 h-8 text-blue-500" />;
        ...
        <div className="flex items-center flex-wrap gap-2 text-2xl font-bold">
          {searchQuery.trim().length >= 2 ? (
            <span className="text-gray-800">
              Kết quả tìm kiếm cho <span className="text-blue-600 italic">"{searchQuery}"</span>
            </span>
          ) : (
          ...
          {searchQuery.trim().length >= 2 ? (
            searchResults.files.length === 0 && searchResults.folders.length === 0 ? (
              <div className="col-span-full flex flex-col items-center justify-center py-20 bg-gray-50/50 rounded-2xl border-2 border-dashed border-gray-200">
                ...
                <p className="text-gray-400 text-sm mt-1">Thử thay đổi từ khóa tìm kiếm của bạn nhé!</p>
              </div>
            ) : (
              <>
                {searchResults.folders.map((folder) => {
                  ...
                  return (
                    <div 
                      key={`search-folder-${folderId}`} 
                      onClick={() => {
                        handleEnterFolder(folder);
                        setSearchQuery('');
                      }} 
                      className="relative border border-blue-100 rounded-2xl p-4 hover:border-blue-300 hover:shadow-md hover:-translate-y-1 transition-all duration-300 cursor-pointer flex items-center gap-4 bg-white group"
                    >
                      ...
                      <div className="flex-1 truncate">
                        ...
                        <p className="text-[10px] text-gray-400">Thư mục</p>
                      </div>
                    </div>
                  );
                })}
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="text-center w-full mt-2 bg-gray-50/50 p-2 rounded-lg">
                        ...
                        <p className="text-xs text-gray-400 mt-0.5 font-medium">{formatFileSize(file.size)}</p>
                      </div>
                    </div>
                  );
                })}
